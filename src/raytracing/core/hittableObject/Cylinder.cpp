...
Raytracing::Cylinder::Cylinder(const glm::vec3& newBaseCenter,
                               float newRadius,
                               float newHeight,
...
double Raytracing::Cylinder::sdf(const glm::vec3& position) const
{
    const glm::vec3 localPosition = inverseRotationMatrix * (position - center);
    const glm::vec2 d = glm::abs(glm::vec2(glm::length(glm::vec2(localPosition.x, localPosition.z)), localPosition.y))
        - glm::vec2(radius, height * 0.5f);
    return glm::min(glm::max(d.x, d.y), 0.f) + glm::length(glm::max(d, glm::vec2(0.f)));
}

glm::vec3 Raytracing::Cylinder::getNormal(glm::vec3 position) const
{
    return estimateNormal(*this, position);
}

bool Raytracing::Cylinder::hasBoundingBox() const
{
    return true;
}

Raytracing::AxisAlignedBoundingBox Raytracing::Cylinder::getBoundingBox() const
{
